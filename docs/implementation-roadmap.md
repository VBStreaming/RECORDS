...
이번 범위에는 사진, 이메일 인증, Refresh Token, 비밀번호 재설정, 푸시 알림, 관리자 기능을 포함하지 않는다. 각 기능은 실제 사용자 검증 또는 공개 배포 조건이 생긴 뒤 별도 계약을 먼저 작성한다.
...
관리자, OCR, AI, 사용자 정의 과목, 반복 일정, 공유 기능은 별도 제품 요구가 생기기 전까지 문서와 구현 모두 확장하지 않는다.
